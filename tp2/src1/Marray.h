...
public:
	...
	//friend ostream& operator<<(ostream& out, Marray<T>& obj);
};
...
template <class T>
void Marray<T>::sortbymarks() {
	for (int i = 1; i < size; i++)
		for (int j = 0; j < size - i; j++)
			if (data[j].getAverageMark() > data[j + 1].getAverageMark())
				std::swap(data[j], data[j + 1]);
}
...
//ostream& operator<<(ostream& out, Marray<T>& obj)
//{
//	out << "ФИО: " << obj.secondName << " " << obj.iniciales << endl;
//	out << "Номер группы: " << obj.numberGroup << endl;
//	out << "Оценки: ";
